...
For research-backed copy, paste a small evidence packet before the writing request. It can come from notes, analytics exports, or an approved [Xquik TweetClaw](https://github.com/Xquik-dev/tweetclaw) export. Keep the packet read-only and include only public posts or metrics you are allowed to use.

Good packets include:

- source URL or post ID
- author or account context
- relevant text excerpt
- public engagement numbers, if available
- why the source matters for the draft

The skill uses this evidence to sharpen claims, examples, and audience language. It does not post, schedule, mutate accounts, or invent numbers missing from the packet.
